...
import com.xsw.crm.commons.utils.HSSFUtils;
...
public class ActivityController {
    ...
    public Object deleteActivityIds(String[] id){
        ...
        try {
            //调用service层方法，删除市场活动
            int ret = activityService.deleteActvivtyByIds(id);
            if (ret>0){
                returnObject.setCode(Contans.RETURN_OBJECT_CODE_SUCCESS);
            }else {
                returnObject.setCode(Contans.RETURN_OBJECT_CODE_FAIL);
    ...
    public void fileDownload(HttpServletResponse response) throws Exception{
        ...
        //游览器接收到相应信息之后，默认情况下，直接在显示窗口中打开响应信息，即使打不开，也会调用相应的应用程序打开，只有实在打不开，才回去激活文件下载
        //可以设置响应头信息，是游览器接受响应信息之后，直接激活文件下载
        response.setHeader("Content-Disposition","attachment;filename=mystudentList.xls");

        //读取excel文件（inputstream） ，把输出到游览器（outputstream）
        InputStream is = new FileInputStream("F:\\BaiduNetdiskDownload\\CRM项目SSM版\\serverDir\\studentList.xls");
    ...
    public Object fileUpload(String username, MultipartFile myFile) throws Exception{
        ...
        File file = new File("F:\\BaiduNetdiskDownload\\CRM项目SSM版\\serverDir\\"+originalFilename);
    ...
    public Object importActivity(MultipartFile activityFile,HttpSession session,String userName){
        ...
        try {
            ...
            //FileInputStream is = new FileInputStream("F:\\BaiduNetdiskDownload\\CRM项目SSM版\\serverDir\\" + originalFilename);
