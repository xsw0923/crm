			...
			$.ajax({
				...
				success:function (data){
					if (data.code=="1"){
						...
						htmlStr+="<h5>"+data.retDate.noteContent+"</h5>";
						htmlStr+="<font color=\"gray\">市场活动</font> <font color=\"gray\">-</font> <b>${activity.name}</b> <small style=\"color: gray;\"> "+data.retDate.createTime+" 由${sessionScope.sessionUser.name}创建</small>";
			...
			$.ajax({
				...
				success:function (data) {
					if(data.code=="1"){
						...
						//刷新备注列表
						$("#div_"+data.retDate.id+" h5").text(data.retDate.noteContent);
						$("#div_"+data.retDate.id+" small").text(" "+data.retDate.editTime+" 由${sessionScope.sessionUser.name}修改");
					}else{
						//提示信息
						alert(data.message);
						//模态窗口不关闭
